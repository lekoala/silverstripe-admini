...
class SiteConfig extends DataObject implements PermissionProvider, TemplateGlobalProvider
{
    ...
    {
        $class = \SilverStripe\SiteConfig\SiteConfigLeftAndMain::class;
        if (Controller::curr() instanceof \LeKoala\Admini\LeftAndMain) {
            $class = \LeKoala\Admini\SiteConfigLeftAndMain::class;
        }
        return $class::singleton()->Link();
    }
    ...
    public function canEditPages($member = null)
    {
        ...
        $canEdit = $this->CanEditType === 'LoggedInUsers'
    ...
    public function canCreateTopLevel($member = null)
    {
        ...
        $canCreate = $this->CanCreateTopLevelType === 'LoggedInUsers'
            && Permission::checkMember($member, $this->config()->get('required_permission'));
        if ($canCreate) {
            return true;
        }
        ...
        // check for specific groups
        $canCreateGroups = $this->CanCreateTopLevelType === 'OnlyTheseUsers'
            && $member
